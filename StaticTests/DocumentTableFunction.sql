...
DECLARE @ModuleInputName	NVARCHAR(MAX) = '[ibi].[getDateListForPeriod]'
...
PRINT 'h1. Abstract';
PRINT CONCAT('{excerpt}&#91;Table-valued Function&#93; ', ISNULL(@ModuleExcerpt, CONCAT(DB_NAME(), '.', @SchemaName, '.', @ModuleName)), '{excerpt}');
PRINT '';
...
SELECT
	...
	, CONVERT(NVARCHAR(MAX), P.default_value)
FROM
	sys.parameters P
...
WHILE (@LoopCurrent <= @LoopMax) BEGIN
	SELECT
		...
		, @ArgumentList = CONCAT(@ArgumentList, CHAR(13), CHAR(10), '| ', PL.ParamName, ' | ', PL.ParamType, PL.LengthDefinition, ' | ', IIF(PL.IsNullable = 0, 'NOT NULL', 'NULL'), ' | ', PL.DefaultValue, ' | ', IIF(PL.IsReadOnly = 1, 'READ_ONLY', '-'), ' | ', IIF(PL.IsOutput = 1, 'OUTPUT', '-'), ' | ', PL.Description, ' |')
...
SET @Syntax = CONCAT(@Syntax, CHAR(13), CHAR(10), ')');
...
PRINT 'h1. Return Table';
PRINT '';
PRINT '|| Column Name || Data Type || Nullable || Default || Description ||'
