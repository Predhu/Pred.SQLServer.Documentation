...
DECLARE @InputTableName NVARCHAR(MAX) = '[calendar].[Calendar]';
...
IF (@PrimaryKey IS NOT NULL and @PrimaryKey != '') BEGIN
	PRINT '';
	PRINT @PrimaryKey;
END
...
IF (EXISTS (SELECT * FROM @UniqueKeyList)) BEGIN
	SET @LoopCurrent	= 1;
