...
DECLARE @ModuleInputName	NVARCHAR(MAX) = '[ibi].[getCalendarWeekKey]'
...
PRINT 'h1. Abstract';
PRINT CONCAT('{excerpt}&#91;Scalar-valued Function&#93; ', ISNULL(@ModuleExcerpt, CONCAT(DB_NAME(), '.', @SchemaName, '.', @ModuleName)), '{excerpt}');
PRINT '';
...
PRINT 'h1. Syntax';
PRINT CONCAT('{code:language=sql}', @Syntax, CHAR(13), CHAR(10), '){code}');
PRINT '';


DECLARE @ReturnValue NVARCHAR(MAX);
SELECT
	@ReturnValue = CONCAT('h2. ', PL.ParamName, '
^', PL.ParamType, PL.LengthDefinition, IIF(PL.IsNullable = 0, ' | NOT NULL', ''), '^
', PL.Description, '
', PL.Remarks
)
FROM
...
WHERE
	...
	AND OBJECT_SCHEMA_NAME(referenced_object_id) IS NOT NULL
	AND OBJECT_NAME(referenced_object_id) IS NOT NULL
...
WHERE
	...
	AND OBJECT_SCHEMA_NAME(parent_object_id) IS NOT NULL
	AND OBJECT_NAME(parent_object_id) IS NOT NULL
...
WHERE
	object_id = @ObjectID
	AND OBJECT_SCHEMA_NAME(referenced_major_id) IS NOT NULL
	AND OBJECT_NAME(referenced_major_id) IS NOT NULL
...
WHERE
	...
	AND OBJECT_SCHEMA_NAME(referencing_id) IS NOT NULL
